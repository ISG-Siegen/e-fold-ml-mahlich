...
max_depth_factor_list = [1, 3, 4, 5, 10]
min_samples_split_list = [2, 6, 10, 14, 18, 20]
...
for iteration in range(1, 26):
    ...
    for max_depth in max_depth_factor_list:
        for min_samples_split in min_samples_split_list:
            ...
            model = DecisionTreeRegressor(
                max_depth=max_depth, min_samples_split=min_samples_split, random_state=random_state_value)
            ...
            run = run + 1
            print(f"Run: {run}")
            print(f"Hyperparameter:  max_depth={max_depth}, min_samples_split={min_samples_split}")
                    ...
                    if std_count == 2:  # Wenn 2 mal hintereinander geringere std und die nachfolgende ebenfalls dann aufhören
                        if check == True:
                            ...
                            if mean_scores < best_score_efold:
                                ...
                                best_params_efold = {
                                    # 'criterion': criterion,
                                    'max_depth': max_depth, 'min_samples_split': min_samples_split, 'run': run}

                            cancelation_fold = fold_idx
                            folds_needed[run] = cancelation_fold
                            all_scores_efold[run] = {
                                # 'criterion': criterion,
            ...
            all_scores_kfold[run] = {
                # 'criterion': criterion,
                'max_depth': max_depth, 'min_samples_split': min_samples_split, 'mean_score': mean_scores}
    ...
    best_model_efold = DecisionTreeRegressor(
    ...
    best_model_kfold = DecisionTreeRegressor(
