...
for iteration in range(1, 26):
    ...
    for fit_intercept in fit_intercept_list:
        for copy_X in copy_X_list:
            for positive in positiv_list:
                ...
                run = run + 1
                print(f"Run: {run}")
                print(f"Hyperparameter: fit_intercept={fit_intercept}, copy_X={copy_X}, positive={positive}")
                        ...
                        if std_count == 2:  # Wenn 2 mal hintereinander geringere std und die nachfolgende ebenfalls dann aufhören
                            if check == True:
                                ...
                                if mean_scores < best_score_efold:
                                    ...
                                    best_params_efold = {'fit_intercept': fit_intercept, 'copy_X': copy_X,
                                                         'positive': positive, 'run': run}

                                cancelation_fold = fold_idx
                                folds_needed[run] = cancelation_fold
                                all_scores_efold[run] = {'fit_intercept': fit_intercept, 'copy_X': copy_X,
                                                         'positive': positive, 'mean_score': mean_scores}
                ...
                if mean_scores < best_score_kfold:
                    ...
                    best_params_kfold = {'fit_intercept': fit_intercept, 'copy_X': copy_X, 'positive': positive,
                                         'run': run}

                all_scores_kfold[run] = {'fit_intercept': fit_intercept, 'copy_X': copy_X, 'positive': positive,
    ...
    best_model_efold = LinearRegression(fit_intercept=best_params_efold['fit_intercept'],
                                        copy_X=best_params_efold['copy_X'],
                                        positive=best_params_efold['positive'])
    ...
    best_model_kfold = LinearRegression(fit_intercept=best_params_kfold['fit_intercept'],
                                        copy_X=best_params_kfold['copy_X'],
                                        positive=best_params_kfold['positive'])
