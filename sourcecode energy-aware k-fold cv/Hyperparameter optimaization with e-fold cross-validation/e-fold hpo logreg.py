...
for iteration in range(1, 26):
    ...
    # Speichern der besten Ergebnisse
    best_score_efold = 0
    best_score_kfold = 0
    best_params_efold = {}
    best_params_kfold = {}
    ...
    for C in C_list:
        for solver in solver_list:
            for max_iter in max_iter_list:
                ...
                run = run + 1
                print(f"Run: {run}")
                print(f"Hyperparameter: C={C},  solver={solver}, max_iter={max_iter}")
                        ...
                        if std_count == 2:  # Wenn 2 mal hintereinander geringere std und die nachfolgende ebenfalls dann aufhören
                            if check == True:
                                ...
                                if mean_scores > best_score_efold:
                                    ...
                                    best_params_efold = {'C': C, 'solver': solver, 'max_iter': max_iter, 'run': run}

                                cancelation_fold = fold_idx
                                folds_needed[run] = cancelation_fold
                                all_scores_efold[run] = {'C': C, 'solver': solver, 'max_iter': max_iter,
                ...
                if mean_scores > best_score_kfold:
                    ...
                    best_params_kfold = {'C': C, 'solver': solver, 'max_iter': max_iter, 'run': run}

                all_scores_kfold[run] = {'C': C, 'solver': solver, 'max_iter': max_iter, 'mean_score': mean_scores}
    ...
    best_model_efold = LogisticRegression(C=best_params_efold['C'],
                                          solver=best_params_efold['solver'],
                                          max_iter=best_params_efold['max_iter']
                                          )
    ...
    best_model_kfold = LogisticRegression(C=best_params_kfold['C'],
                                          solver=best_params_kfold['solver'],
                                          max_iter=best_params_kfold['max_iter'])
